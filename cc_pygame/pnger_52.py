...
def get_shades_combinations(color1: pygame.Color) -> list[pygame.Color]:
    return [shade_to_neutral(color1, 100, get_rev_color(color1)),
            shade_to_neutral(get_rev_color(color1), 100, color1),
            shade_to_neutral(get_rev_color(color1), 100),
            shade_to_neutral(color1, 100),
            shade_to_neutral(get_rev_color(color1), 100, pygame.Color("black")),
            shade_to_neutral(color1, 100, pygame.Color("black")),
            shade_to_neutral(pygame.Color("black"), 100, color1),
            shade_to_neutral(pygame.Color("white"), 100, color1),
            shade_to_neutral(pygame.Color("black"), 100, get_rev_color(color1)),
            shade_to_neutral(pygame.Color("white"), 100, get_rev_color(color1))]
    ...
    for p in pos:
        ...
        if selection == 0:
            for r in range(1, 101, 1):
                draw_circle(screen, f"grey{r}", p, radius - r * radius / 100)
        elif selection == 1:
            for r in range(1, 101, 1):
                draw_circle(screen, f"grey{101 - r}", p, radius - r * radius / 100)
        elif selection >= 2 and selection <= len(shades_list) + 1:
            for r in range(1, 101, 1):
                draw_circle(screen, shades_list[selection - 2][r], p, radius - r * radius / 100)
        else:
            continue
    pygame.image.save(screen, 'cc_pygame/gen/jpger_52.jpg')
    pygame.quit()
    return 0
